/**
 * /api/hr-people/payroll/[id]/confirm
 *
 * Confirm payroll run (locks it)
 */
 ...
 * Confirm payroll run (changes status to confirmed)
 */
  ...
  try {
    ...
    if (run.status === "posted_to_finance") {
      return NextResponse.json(
        { error: "Payroll run has already been posted to finance" },
        { status: 400 }
      );
    }
    ...
    // Update each line with final values
    let totalGross = 0;
    ...
    return NextResponse.json({
      ...
      totals: {
        ...
        total_net: totalNet,
        total_tax: totalTax,
        employee_count: body.lines.length,
      },
    });
  } catch (error) {
    ...
    console.error("POST /api/hr-people/payroll/[id]/confirm error:", error);
    return NextResponse.json(
