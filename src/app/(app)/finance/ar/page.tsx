...
export default function ARPage() {
  ...
  const loadOpen = React.useCallback(async () => {
    ...
    try {
      ...
      const result = await apiGet<OpenDocsResult>(`/api/finance/ar/open?${params}`);
      setOpenData(result);
    } catch (err) {
  ...
  const loadStatement = React.useCallback(async () => {
    ...
    try {
      ...
      const result = await apiGet<StatementResult>(`/api/finance/ar/statement?${params}`);
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Accounts Receivable" description="Customer balances and statements" />
        ...
        <>
          <GlassCard padding="sm">
            <div className="flex items-center gap-6">
              ...
              <div>
                <span className="text-xs text-white/50 uppercase">Total Open</span>
          ...
          <GlassCard padding="none">
            <GlassTable
              headers={["Invoice #", "Date", "Customer", "Total", "Paid", "Remaining"]}
              monospaceColumns={[0]}
      ...
      {tab === "statement" && statementData && (
        <>
          <GlassCard padding="sm">
            <div className="flex items-center gap-6">
              <div>
                <span className="text-xs text-white/50 uppercase">Customer</span>
                <p className="text-lg font-medium text-white">{statementData.partyName || statementData.partyId}</p>
              </div>
              <div>
                <span className="text-xs text-white/50 uppercase">Closing Balance</span>
                <p className="text-xl font-semibold text-emerald-400">
